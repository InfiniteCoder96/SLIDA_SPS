            ...
            <div class="row">
                <div class="col-md-12">
                    @if($transcript_requests)
                        <div class="card">
                            <div class="card-body">
                                <h5 class="card-title m-b-0">Result Information<span class="label label-info" style="font-size: 20px">{{count($transcript_requests)}}</span></h5>
                            </div>
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    @foreach($transcript_requests as $transcript_request)

                                        <tr>

                                            <td style="font-size: 12px">{{$transcript_request->sid}}</td>
                                            <td style="font-size: 12px">{{$transcript_request->data['full_name']}}</td>
                                            <td style="font-size: 12px">{{$transcript_request->data['NIC']}}</td>
                                            <td style="font-size: 12px">{{$transcript_request->data['email']}}</td>
                                                ...
                                                <div class="form-row">
                                                    ...
                                                    <div class="col-md-4">
                                                        <form action="{{url('/admin/officialRequests/showPostponementLetter')}}" method="post" >
                                                            {{csrf_field()}}
                                                            <input name="id" type="hidden" value="{{$transcript_request->id}}">
